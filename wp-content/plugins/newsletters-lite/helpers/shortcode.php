...
if (!class_exists('wpmlShortcodeHelper')) {
	class wpmlShortcodeHelper extends wpMailPlugin {
		...
		function posts_sendas($atts = array(), $content = null) {
			global $post, $shortcode_posts, $shortcode_sendas, $shortcode_post_language, $shortcode_post_showdate;
		...
		function posts_loop_wrapper($atts = array(), $content = null, $tag = null) {
			...
			if (!empty($shortcode_categories)) {	
				...
				foreach ($shortcode_sendas as $post) {				
					$shortcode_post = $post;				
					$return .= do_shortcode($this -> et_message('sendas', false, $shortcode_post_language));
					$shortcode_post_row++;
				}
