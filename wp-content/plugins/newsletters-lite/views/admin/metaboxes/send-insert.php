	...
	<?php if (!empty($inserttabs['fields'])) : ?>
		<div id="inserttabs-1">
	        ...
	        <?php if (!empty($fields)) : ?>
	        	<ul class="insertfieldslist">
		            <?php foreach ($fields as $field) : ?>
		            	<li>
		            		<a href="" class="press button button-secondary" onclick='wpml_tinymcetag("[<?php echo $this -> pre; ?>field name=\"<?php echo $field -> slug; ?>\"]"); return false;'><?php echo __($field -> title); ?></a>
		            	</li>
		            <?php endforeach; ?>
	        	</ul>
	        <?php endif; ?>
	        <?php if (!empty($Subscriber -> table_fields)) : ?>
	        	<p>
		        	<a href="" onclick="jQuery('#morefieldslist').toggle(); return false;" class="button button-primary"><?php _e('More Fields', $this -> plugin_name); ?></a>
	        	</p>
		        	...
		        	<ul class="insertfieldslist">
			        	<?php foreach ($Subscriber -> table_fields as $field => $attributes) : ?>
			        		<?php if ($field != "email" && $field != "key") : ?>
				        		<li>
				        			<a href="" class="press button button-secondary" onclick='wpml_tinymcetag("[<?php echo $this -> pre; ?>field name=\"<?php echo $field; ?>\"]"); return false;'><?php echo $Field -> title_by_slug($field); ?></a>
				        		</li>
				        	<?php endif; ?>
			        	<?php endforeach; ?>
		        	</ul>
	...
	<?php if (!empty($inserttabs['posts'])) : ?>
		<div id="inserttabs-2">
			...
			function get_posts() {
				...
				jQuery.ajax(wpmlajaxurl + '?action=wpmlgetposts', {
					type: 'POST',
					data: arguments,
