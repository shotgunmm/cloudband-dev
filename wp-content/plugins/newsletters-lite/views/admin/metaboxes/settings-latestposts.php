		...
		<?php foreach ($latestpostssubscriptions as $latestpostssubscription) : ?>
			<tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>" id="latestposts_row_<?php echo $latestpostssubscription -> id; ?>">
				...
				<td>
					...
					<?php endif; ?>
					<a href="" title="<?php echo esc_attr(stripslashes($latestpostssubscription -> subject)); ?>" onclick="jQuery(this).colorbox({iframe:true, width:'80%', height:'80%', href:wpmlajaxurl + '?action=wpmllatestposts_preview&id=<?php echo $latestpostssubscription -> id; ?>'}); return false;" class="button"><i class="fa fa-eye fa-fw"></i></a>
