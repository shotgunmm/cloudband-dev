...
if (!class_exists('wpMailPlugin')) {
	class wpMailPlugin extends wpMailCheckinit {
		...
		var $models = array('Link', 'Click', 'Subscribeform', 'Option', 'SubscribersOption', 'Content', 'Latestpostssubscription', 'Order', 'Template', 
		'Group', 'Autoresponder', 'Autoresponderemail', 'FieldsForm');
	    ...
	    function ajax_forms_createform() {
		    ...
		    if (!empty($_POST)) {
			    if ($this -> Subscribeform() -> save($_POST[$this -> Subscribeform() -> model])) {
				    $success = true;
				    //$this -> redirect(admin_url('admin.php?page=' . $this -> sections -> forms . '&amp;method=save&amp;id=' . $this -> Subscribeform() -> insertid));
			    }
		...
		function autoresponders_send($subscriber = null, $mailinglist = null) {
			global $wpdb, $Db, $AutorespondersList, $History, $HistoriesAttachment, $Subscriber, $SubscribersList, $Html, $Email;
		...
		function sc_management($atts = array(), $content = null) {
			global $wpdb, $Db, $Subscriber, $SubscribersList, $Auth, $Field, $Unsubscribe, 
			$newsletters_errors;
		...
		function render_field($field_id = null, $fieldset = true, $optinid = null, $showcaption = true, $watermark = true, $instance = null, $offsite = false, $errors = array()) {
			...
			if (!empty($field_id)) {
				if ($field = $Field -> get($field_id)) {	
					...
					$list = (empty($instance['list'])) ? __($_REQUEST['list_id'][0]) : __($instance['list']);
					...
					echo '<div class="form-group' . ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' has-error' : '') . '">';
					...
					switch ($field -> type) {
						case 'hidden'			:						
							switch ($field -> hidden_type) {
								...
								case 'custom'				:
								default  					:
									$hidden_value = $_REQUEST[$field -> slug];
									break;
								case 'get'					:
							...
							break;
						case 'text'				:						
							echo '<input' . $placeholder . ' class="form-control ' . $this -> pre . ' ' . $this -> pre . 'text' . ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : '') . '" id="' . $this -> pre . '-' . $optinid . '' . $field -> slug . '" ' . $Html -> tabindex($optinid) . ' type="text" name="' . $field -> slug . '" value="' . esc_attr(stripslashes($_REQUEST[$field -> slug])) . '" />';
							break;
						case 'special'			:						
							switch ($field -> slug) {
								case 'list'				:
											...
											if (is_numeric($list)) {
												...
												if ($list == "checkboxes") {
													foreach ($lists as $list_id => $list_title) {
														...
														echo '<input' . ((!empty($_REQUEST['list_id']) && in_array($list_id, $_REQUEST['list_id'])) ? ' checked="checked"' : '') . ' type="checkbox" name="list_id[]" value="' . $list_id . '" class="newsletters-list-checkbox" id="' . $optinid . $field -> slug . '-list-checkbox" /> ';
													...
													foreach ($lists as $list_id => $list_title) {
														echo '<option' . ((!empty($_REQUEST['list_id']) && $_REQUEST['list_id'][0] == $list_id) ? ' selected="selected"' : '') . ' value="' . $list_id . '">' . __($list_title) . '</option>';
													}
							...
							break;
						case 'textarea'			:
							echo '<textarea' . $placeholder . ' class="form-control ' . $this -> pre . ' ' . $this -> pre . 'textarea' . ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : '') . '" id="' . $this -> pre . '-' . $optinid . '' . $field -> slug . '" ' . $Html -> tabindex($optinid) . ' rows="4" name="' . $field -> slug . '">' . strip_tags($_REQUEST[$field -> slug]) . '</textarea>';
							break;
						case 'select'			:						
							echo '<select' . $placeholder . ' class="form-control ' . $this -> pre . ' ' . $this -> pre . 'select' . ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : '') . '" style="width:auto;" id="' . $this -> pre . '-' . $optinid . '' . $field -> slug . '" ' . $Html -> tabindex($optinid) . ' name="' . $field -> slug . '">';
							echo '<option value="">' . __('- Select -', $this -> plugin_name) . '</option>';
							
							if (!empty($field -> newfieldoptions)) {
								foreach ($field -> newfieldoptions as $option_id => $option_value) {
									$select = (!empty($_REQUEST[$field -> slug]) && ($_REQUEST[$field -> slug] == $option_id || $_REQUEST[$field -> slug] == __($option_value))) ? 'selected="selected"' : '';
						...
						case 'radio'			:							
							if (!empty($field -> newfieldoptions)) {
								foreach ($field -> newfieldoptions as $option_id => $option_value) {																		
									$checked = ($_REQUEST[$field -> slug] == $option_id || (!empty($_REQUEST[$field -> slug]) && $_REQUEST[$field -> slug] == __($value))) ? 'checked="checked"' : '';
									echo '<div class="radio">';
									echo '<label class="control-label wpmlradiolabel ' . $this -> pre . '">';
									echo '<input class="' . $this -> pre . 'radio' . ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : '') . '" ' . $Html -> tabindex($optinid) . ' type="radio" ' . $checked . ' name="' . $field -> slug . '" value="' . $option_id . '" /> ' . __($option_value);
									echo '</label>';
									echo '</div>';
								}
							}
						...
						case 'checkbox'			:							
							if (!empty($field -> newfieldoptions)) {
								foreach ($field -> newfieldoptions as $option_id => $option_value) {
									$checked = (!empty($_REQUEST[$field -> slug]) && (is_array($_REQUEST[$field -> slug]) && in_array($option_id, $_REQUEST[$field -> slug]))) ? 'checked="checked"' : '';
									echo '<div class="checkbox">';
									echo '<label class="control-label wpmlcheckboxlabel ' . $this -> pre . '">';
									echo '<input class="' . $this -> pre . 'checkbox' . ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : '') . '" ' . $Html -> tabindex($optinid) . ' type="checkbox" ' . $checked . ' name="' . $field -> slug . '[]" value="' . $option_id . '" /> ' . __($option_value);
									echo '</label>';
									echo '</div>';
								}
							}
						...
						case 'file'				:						
							...
							<?php /*<input type="file" <?php echo $Html -> tabindex($optinid); ?> name="file_upload_<?php echo $field -> id; ?>" id="file_upload_<?php echo $field -> id; ?><?php echo $optinid; ?>" />
							<input type="hidden" name="<?php echo $field -> slug; ?>" value="<?php echo esc_attr(stripslashes($_REQUEST[$field -> slug])); ?>" id="<?php echo $this -> pre . '-' . $optinid . '' . $field -> slug; ?>" />*/ ?>
						...
						case 'pre_country'		:
							...
							if ($countries = $wpmlCountry -> select()) {						
								...
								<select class="<?php echo $this -> pre; ?>country <?php echo $this -> pre; ?> <?php echo $this -> pre; ?>precountry<?php echo ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : ''); ?>" id="<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo $field -> slug; ?>">
									<option value=""><?php _e('- Select Country -', $this -> plugin_name); ?></option>
									<?php foreach ($countries as $id => $value) : ?>
										<option <?php echo (!empty($_REQUEST[$field -> slug]) && $_REQUEST[$field -> slug] == $id) ? 'selected="selected"' : ''; ?> value="<?php echo $id; ?>"><?php echo $value; ?></option>
									<?php endforeach; ?>
								</select>
						...
						case 'pre_date'			:
							...
							$field_value = false;
							if (!empty($_REQUEST[$field -> slug])) {
								$field_value = maybe_unserialize($_REQUEST[$field -> slug]);
							}
							...
							<input type="text" class="<?php echo $this -> pre; ?>predate <?php echo $this -> pre; ?>text <?php echo $this -> pre; ?> <?php echo $this -> pre; ?>predate<?php echo ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : ''); ?>" value="<?php echo $currentDate; ?>" name="<?php echo $field -> slug; ?>" id="<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>" />
						...
						case 'pre_gender'		:
							...
							<select<?php echo $placeholder; ?> class="<?php echo $this -> pre; ?> <?php echo $this -> pre; ?>pregender<?php echo ((!empty($_REQUEST[$this -> pre . 'errors'][$field -> slug])) ? ' ' . $this -> pre . 'fielderror' : ''); ?>" style="width:auto;" id="<?php echo $this -> pre; ?>-<?php echo $optinid; ?><?php echo $field -> slug; ?>" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo $field -> slug; ?>">
								<option value=""><?php _e('- Select Gender -', $this -> plugin_name); ?></option>
								<option <?php echo (!empty($_REQUEST[$field -> slug]) && $_REQUEST[$field -> slug] == "male") ? 'selected="selected"' : ''; ?> value="male"><?php _e('Male', $this -> plugin_name); ?></option>
								<option <?php echo (!empty($_REQUEST[$field -> slug]) && $_REQUEST[$field -> slug] == "female") ? 'selected="selected"' : ''; ?> value="female"><?php _e('Female', $this -> plugin_name); ?></option>
							</select>
					...
					if (!empty($field -> type) && $field -> type == "file") {
						...
						if (!empty($_REQUEST[$field -> slug])) {
							echo $Html -> file_custom_field($_REQUEST[$field -> slug], $field -> filesizelimit, $filetypes);
							echo '<input type="hidden" name="oldfiles[' . $field -> slug . ']" value="' . esc_attr(stripslashes($_REQUEST[$field -> slug])) . '" />';
						}
					}
		...
		function process_set_variables($subscriber = null, $user = null, $message = null, $history_id = null, $eunique = null, $issubject = false) {
			global $wpdb, $Db, $wpmlCountry, $Field, $Mailinglist, $Subscriber, $Html, $History, $Theme;
			...
			if (!empty($message)) {	
				...
				if (!empty($subscriber)) {	
					...
					$pattern = "/\[(\[?)(wpmlfield)(?![\w-])([^\]\/]*(?:\/(?!\])[^\]\/]*)*?)(?:(\/)\]|\](?:([^\[]*+(?:\[(?!\/\2\])[^\[]*+)*+)\[\/\2\])?)(\]?)/s";
					$message = preg_replace_callback($pattern, array($this, 'replace_custom_field'), $message);
		...
		function execute_mail($subscriber = null, $user = null, $subject = null, $message = null, $attachments = null, $history_id = null, $eunique = null, $shortlinks = true) {
			global $wpdb, $Db, $Html, $Email, $History, $phpmailer, $Mailinglist, $Subscriber, 
		...
		function get_latestposts($latestpostssubscription = null) {
			global $wpdb, $post, $Db, $Latestpost, $Html, $History, $Mailinglist, $Queue, $Subscriber, $SubscribersList;
			$post_criteria = false;
		...
		function updating_plugin() {
			...
			if (version_compare($this -> version, $cur_version) === 1) {				
				if (version_compare("3.8.4", $cur_version) === 1) {											
					...
					global $wpdb, $History, $Theme, $Queue;
					$wpdb -> flush();
				...
				if (version_compare($cur_version, "4.5.7") < 0) { 
					$this -> update_options();
					$version = "4.5.7";	
				}
