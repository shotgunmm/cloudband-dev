...
if (!class_exists('wpmlcf7')) {
	...
	if (class_exists('wpMailPlugin')) {
		class wpmlcf7 extends wpMailPlugin {
			...
			function wpcf7_add_shortcode_newsletters() {
				if (function_exists('wpcf7_add_shortcode')) {
					wpcf7_add_shortcode(array('newsletters'), array($this, 'wpcf7_newsletters_shortcode_handler'), true);
				}
			}
			
			function wpcf7_newsletters_shortcode_handler($tag = null) {	
				global $Mailinglist;
					...
					foreach ($tags as $tag) {				
						if ($tag['basetype'] == "newsletters") {
							...
							if (!empty($tag -> name)) {							
								...
								if (!empty($posted_data[$tag -> name]) && ($posted_data[$tag -> name] == 1 || is_array($posted_data[$tag -> name]))) {							
									$dosubscribe = true;
		...
		add_action('init', array($wpmlcf7, 'wpcf7_add_shortcode_newsletters'), 10, 1);
